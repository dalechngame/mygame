...
void LayerGame::loadResource()
{
	...
	{
		CocosDenshion::SimpleAudioEngine::sharedEngine()->preloadEffect(effectMusicfile[i]);
		CocosDenshion::SimpleAudioEngine::sharedEngine()->preloadBackgroundMusic(bgMusicFile[i]);
	}
...
void LayerGame::startGame()
{
	...
	isEff ? CocosDenshion::SimpleAudioEngine::sharedEngine()->resumeAllEffects() :
		CocosDenshion::SimpleAudioEngine::sharedEngine()->pauseAllEffects();
	isBg ?// 循环播放
		CocosDenshion::SimpleAudioEngine::sharedEngine()->playBackgroundMusic("OnLand.wav", true) :
...
void LayerGame::GameOver(Common::OVERTYPE type)
{
	Mario::_life -= 1;
	if (Mario::_life == 0)
	{
		CCDirector::sharedDirector()->replaceScene(CCTransitionCrossFade::create(0.5f, LayerOver::scene(type)));
	}
	else
	{
		CocosDenshion::SimpleAudioEngine::sharedEngine()->playBackgroundMusic("OnLand.wav");
		_map->setPositionX(0);
		CCPoint birthPoint = ccp(32, 176);
		_mario->stopAllActions();
		_mario->setPosition(birthPoint);
		_mario->_status = Mario::NORMAL;
	}
}
...
void LayerGame::fireCallBack(CCObject*)
{
	...
	CC_SAFE_RELEASE(Item::_pbulletArray);
	GameInfo::Utime = 300;
	LayerGame::_idx++;
...
void LayerGame::openSchedule()
{
	...
	schedule(schedule_selector(LayerGame::checkTimeOut));
}
...
void LayerGame::checkTimeOut(float)
{
	if (GameInfo::Utime <= 0)
	{
		CCDirector::sharedDirector()->replaceScene(CCTransitionCrossFade::create(0.5f, LayerOver::scene(Common::TIME_OUT)));
	}
