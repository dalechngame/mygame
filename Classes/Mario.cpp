		...
		for (int j = 0; j < 4; ++j)
		{
			...
			if (gid != 0)
			{
				...
				if (_bAutoRun && !_bflagRunAction)
				{
					...
					game->audioEngine->playEffect("QiZiLuoXia.wav");
				}
...
void Mario::updateStatus()
{
	...
	if (_status == SHOOT)
	{
		...
		CCSequence *seq = CCSequence::create(ani, call, nullptr);
		runAction(seq);
		return;
	}
	if (_status == CHANGSTA)
	{
		...
		CCSequence * seq = CCSequence::create(blink, call, nullptr);
	...
	if (_status == DEAD)
	{
		float jpHeight = ((CCTMXTiledMap *)getMap())->getContentSize().height / 5;
		CCAnimate*animate = nullptr;
		if (_grow == NONE)
		{
		...
		CCJumpBy* moveUp = CCJumpBy::create(1.0f, CCPointZero, jpHeight, 1);
		CCMoveBy* moveDown = CCMoveBy::create(2.0 / 3, ccp(0, -winSize.height));
...
void Mario::eatMushroom(Item::ItemType type)
{
	...
	{
		LayerGame *game = (LayerGame *)(getMap()->getParent());
		game->audioEngine->playEffect("EatMushroomOrFlower.wav");
		if (_grow == NONE)
		{
	...
	{
		LayerGame *game = (LayerGame *)(getMap()->getParent());
		game->audioEngine->playEffect("AddLife.wav");
		_life++;
	}
}
...
void Mario::hit(CCTMXLayer* layer, int gid, CCPoint ptTile)
{
	...
	{
		LayerGame *game = (LayerGame *)(getMap()->getParent());
		game->audioEngine->playEffect("DingYingZhuanKuai.wav");
		return;
	}
	...
	if (gid == 431 || gid == 846 || gid == 601)
	{
		...
		if (gid == 601 && !_hasReward(sprite))
		{
			...
			// 播放blinkcoin的动画
			{
				...
				game->coin->runAction(CCSpawn::create(jump, CCRepeat::create(animate, 2), nullptr));
				game->audioEngine->playEffect("EatCoin.wav");
			}
...
bool Mario::_hasReward(CCNode* sprite)
{
	...
	CCARRAY_FOREACH(Item::_itemReward, obj)
	{
		...
		{
			LayerGame *game = (LayerGame *)(getMap()->getParent());
			game->audioEngine->playEffect("DingChuMoGuHuoHua.wav");
			item->wakeup();
			return true;
		}
	}
...
void Mario::die(bool dead)
{
	...
	if (dead)
	{
		_status = DEAD;
		LayerGame *game = (LayerGame *)(getMap()->getParent());
		game->audioEngine->playEffect("DiaoRuXianJingSi.wav");
		game->audioEngine->stopBackgroundMusic("OnLand.wav");
	...
	if (dead == false && (_grow == BIG || _grow == FIRE))
	{
		...
		CCTMXTiledMap *map = getMap();
		LayerGame *game = (LayerGame *)(map->getParent());
		game->audioEngine->playEffect("SuoXiao.wav");
		game->unschedule(schedule_selector(LayerGame::moveMarioUpd));
	...
	LayerGame *game = (LayerGame *)(getMap()->getParent());
	game->audioEngine->playEffect("YuDaoGuaiWuSi.wav");
	game->audioEngine->stopBackgroundMusic("OnLand.wav");
...
void Mario::setFrame(const char *frameName, Common::DIRECTION dir, _GROW grow, bool isAni)
{
	std::string name = "";
	if (dir == Common::LEFT)
	{
...
void Mario::setAction(const char *actionName, Common::DIRECTION dir, _GROW grow, bool forever)
{
	std::string name = "";
	if (dir == Common::LEFT)
	{
